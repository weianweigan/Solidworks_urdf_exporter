...
[DataContract(
    IsReference = true,
    Name = "Attribute",
    Namespace = "http://schemas.datacontract.org/2004/07/SW2URDF"
)]
public class URDFAttribute
{
    public static readonly NumberFormatInfo URDFNumberFormat = CultureInfo
    ...
    public void WriteURDF(XmlWriter writer)
    {
        ...
        if (Value.GetType() == typeof(double[]))
        {
            ...
            {
                valueString += d.ToString(URDFNumberFormat) + " ";
            }
            valueString = valueString.Trim();
        }
        else if (Value.GetType() == typeof(double))
        {
            valueString = ((Double)Value).ToString(URDFNumberFormat);
        }
        ...
        if (Value.GetType() == typeof(double[]))
        {
            ...
            for (int i = 0; i < values.Length; i++)
            {
                string contextString =
                    string.Join(".", context) + "." + AttributeType + "." + AttributeType[i];
